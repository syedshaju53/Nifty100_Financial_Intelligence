...
def get_company_ratios(company_id: str):
    """
    Return the latest available financial ratios for one company.

    The endpoint is intentionally backed by ``financial_ratios`` so
    the API uses the same normalized ratio dataset as the screener.
    """
    ...
    try:
        ...
        if row is None:
            raise HTTPException(
                status_code=404,
                detail=f"Ratios for company '{company_id.upper()}' not found",
            )
